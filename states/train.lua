...
function state.load()
    ...
    mapData = require("assets.lua.train") 
...
function state.update(dt)
    ...
    for _, layer in ipairs(mapData.layers) do
        -- On cherche le calque des collisions / objets invisibles
        if layer.name == "Collisions" and layer.type == "objectgroup" then
            for _, obj in ipairs(layer.objects) do
                ...
                if checkCollision(p_x, p_y, p_w, p_h, m_x, m_y, m_w, m_h) then
                    if obj.name == "sortie_train" then
                        -- Retour au musée (Coordonnées RÉELLES)
                        change_scene("museum", 200, 150)
                        return
                    else
                        is_colliding = true
                    end
...
function state.draw(screen_name)
    if screen_name ~= "bottom" then
        ...
        love.graphics.print("Lieu : Le train", 20, 20)
        love.graphics.print("Inventaire :", 20, 60)
        love.graphics.setColor(0.6, 0.6, 0.6)
        --love.graphics.print("a", 40, 80) 
